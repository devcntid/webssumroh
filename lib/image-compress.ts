...
export const IMAGE_TARGET_MAX_BYTES = 200 * 1024; // 200KB
 ...
 * Tries quality first, then downscales if still too large.
 */
export async function compressImageToWebp(
  ...
  while (edge >= MIN_EDGE_PX) {
    ...
    while (low <= high) {
      ...
      const encoded = await encodeWebp(source, width, height, longest, edge, quality);

      if (encoded.byteLength <= IMAGE_TARGET_MAX_BYTES) {
        candidate = encoded;
        low = quality + 1; // try higher quality while staying under cap
      } else {
        high = quality - 1;
      }
    }
    ...
    // Still too big at MIN_QUALITY — shrink dimensions and retry
    edge = Math.floor(edge * 0.75);
  }

  if (!best) {
    best = await encodeWebp(source, width, height, longest, MIN_EDGE_PX, MIN_QUALITY);
  }
