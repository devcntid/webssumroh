...
export async function GET(req: NextRequest, ctx: Ctx) {
  ...
  const data = await getContactLeadById(id);
...
export async function PATCH(req: NextRequest, ctx: Ctx) {
  ...
  const existing = await getContactLeadById(id);
  if (!existing) return NextResponse.json({ error: "Not found" }, { status: 404 });

  let data = existing;

  if (parsed.data.status !== undefined) {
    const updated = await updateContactLeadStatus(
      id,
      parsed.data.status,
  ...
  if (parsed.data.agent_notes) {
    const updated = await appendContactLeadAgentNote(id, parsed.data.agent_notes);
    if (!updated) return NextResponse.json({ error: "Not found" }, { status: 404 });
    data = updated;
  }
